...
describe("Main Process - Webview Error Handling (ESM Attempt)", function () {
  ...
  before(() => {
    ...
    mockMainWindowInstanceActual = {
      ...
      on: sinon.stub(),
      webContents: {
        // Fresh webContents mock for each run
        send: sinon.stub(),
        executeJavaScript: sinon.stub().resolves(),
      },
      show: sinon.stub(),
  ...
  it("should register webview IPC handlers (conceptual)", function () {
    // Changed to function
  ...
  it('should update lastGoodUrl on "webview-load-succeeded" (if handler is correct)', function () {
    // Changed to function
  ...
  it('should show error and revert to lastGoodUrl on "webview-load-failed" (if handler is correct)', function () {
    // Changed to function
    if (!ipcCallbackWebviewLoadSucceeded || !ipcCallbackWebviewLoadFailed) this.skip();
  ...
  it('should show error and revert to "about:blank" if no other valid lastGoodUrl (if handler is correct)', function () {
    // Changed to function
  ...
  describe('updateWebviewUrl function (via IPC "update-webview-url", if handler is correct)', function () {
    // Changed to function
    it("updateWebviewUrl should call executeJavaScript with validated URL", function () {
      // Changed to function
